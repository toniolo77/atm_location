...
exports.fixCajero = (req, res, next) =>  {
    Cajero.find({"red": "BANELCO"}).then(elems => {
        elems.forEach(elem => {
            elem.geoloc = {
                "type": "Point", 
                "coordinates": [elem.long,elem.lat]
            };
            elem.save().then(res => {
                console.log("Termino")
            },err => {
...
exports.getCajeros = (req, res, next) =>  {
    if(typeof req.query.red !== 'undefined' && typeof req.query.latitude !== 'undefined' && typeof req.query.longitude !== 'undefined') {
        const type_red = req.query.red,   
            latitude = req.query.latitude,
            longitude = req.query.longitude
            
        Cajero.find({$and : [{geoloc:{$near : { $maxDistance: 1000,$geometry: {type: "Point",coordinates: [longitude,latitude]}}}}, {red: type_red}]}).select('id_app banco ubicacion red lat long').limit(3).then(p => {
